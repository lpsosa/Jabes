...
new_database = function(sepkoski00) {
  ...
  t_dep[ini:dim(t_dep)[1], 10] = rep(x$grupoid[length(x$longevidad)],
                                     x$longevidad[length(x$longevidad)])
  t_dep[ini:dim(t_dep)[1], 8] = rep(x$FA[length(x$longevidad)],
                                    x$longevidad[length(x$longevidad)])
  t_dep[ini:dim(t_dep)[1], 1] = rep(length(x$longevidad),
                                    x$longevidad[length(x$longevidad)])
  if (x$observed[length(x$longevidad)] == TRUE) {
    t_dep[dim(t_dep)[1], 4] = 1
  }
...
#   ____________________________________________________________________________
#   Estimates, standard errors and hazard multipliers per unit covariate    ####
...
simple_cox = function(t_dep) {
  return(
    coxph(
      Surv(t_dep$t_ini, t_dep$t_fin, t_dep$event) ~ t_dep$Sea_level +
        t_dep$sinusoid62 + t_dep$sinusoid140 + t_dep$`fitness` + t_dep$SSTdiff +
        relevel(as.factor(t_dep$g_id), ref = "7"),
      cluster = t_dep$id
    )
  )
}
...
cox_complex = function(t_dep, model_cuts) {
  cut_Cox_mar = survSplit(
    Surv(t_dep$t_ini, t_dep$t_fin, t_dep$event) ~ t_dep$Sea_level +
      t_dep$sinusoid62 + t_dep$sinusoid140 + t_dep$`fitness` +
      t_dep$SSTdiff + relevel(as.factor(t_dep$g_id), ref = "7"),
    data = data.frame(t_dep),
    cut = model_cuts[[1]],
    episode = "tgroup",
    id = "id"
  )
  cut_Cox_sin_2 = survSplit(
    Surv(t_dep$t_ini, t_dep$t_fin, t_dep$event) ~ t_dep$Sea_level +
      t_dep$sinusoid62 + t_dep$sinusoid140 + t_dep$`fitness` +
      t_dep$SSTdiff + relevel(as.factor(t_dep$g_id), ref = "7"),
    data = data.frame(t_dep),
  ...
  cut_Cox_f = cbind(cut_Cox_mar, cut_Cox_sin_2[, 11], cut_Cox_evo[, 11],
                    cut_Cox_grupos[, 11])
  cox = coxph(
    Surv(t_dep$t_ini, t_dep$t_fin, t_dep$event) ~
      t_dep$Sea_level:strata(tgroup) + t_dep$sinusoid62  +
      t_dep$sinusoid140:strata(cut_Cox_sin_2[, 11]) +
      t_dep$`fitness`:strata(cut_Cox_evo[, 11]) +
      t_dep$SSTdiff + as.factor(t_dep$g_id):strata(cut_Cox_grupos[, 11]) ,
    data = cut_Cox_f
  )
...
list_of_coefficients = function(cox, nomb_coef) {
  ...
  for (i in 1:length(nomb_coef)) {
    a = which(grepl(nomb_coef[i], names(cox[[1]]$coefficients), 
                    ignore.case = T) == TRUE)
...
model_cuts = list(c(9, 26, 72),
                  c(),
                  c(6, 17, 42),
                  c(6, 17, 42, 72),
                  c(),
                  c(5, 11, 20, 30, 42))
names(model_cuts) = c("Sea_level",
                      "sinusoid62",
                      "sinusoid140",
                      "fitness",
                      "SSTdiff",
...
execute_cox_complex = function() {
  ...
  for (i in 1:6) {
    names(coeficientes[[6]])[(i * 7 - 6):(i * 7)] = c(
      "Trilobita",
      "Brachiopoda",
      "Gastropoda",
      "Cephalopoda",
...
# Deviance
plot(
  resid(c[[1]], type = "deviance", collapse = base$id),
  xlab = "indice",
  ylab = "residuos(tipo desvio)",
  main = "Residuos (tipo deviance)"
)
summary(resid(c[[1]], type = "deviance", collapse = base$id))

# Martingale
plot(
  resid(c[[1]], type = "martingale", collapse = base$id),
  xlab = "indice",
  ylab = "residuos(tipo martingala)",
  main = "Residuos (tipo martingala)"
)
...
plot(haz_res$time,
     ...
     ylab = "Cumulative hazard of residual",
